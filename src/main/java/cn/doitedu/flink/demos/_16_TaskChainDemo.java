...
public class _16_TaskChainDemo {
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<String> map1 = source.map(s -> s).name("map1");

        SingleOutputStreamOperator<String> filter1 = map1.filter(s -> !s.startsWith("x")).name("filter1");
        ...
        SingleOutputStreamOperator<String> map2 = windowApply.map(s -> s.toUpperCase())/*.setParallelism(2)*//*.slotSharingGroup("x01")*//*.startNewChain()*//*.disableChaining()*/.name("map2");

        map2.print()/*.setParallelism(2)*/.name("print");
